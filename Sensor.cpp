...
void Sensor::onSensorReceived(quint16 ID_A, quint32 ID_B, QList<QByteArray> data)
{
    ...
    {
        _value = (data.at(0)+ data.at(1)).toInt(nullptr, 16)/10; // Why divide it by 10?
        emit valueChanged();
        return;
    }
    else if (_convertedSensorID == data.at(2).toInt(nullptr, 16))
        {
        _value = (data.at(3)+ data.at(4)).toInt(nullptr, 16)/10;
        emit valueChanged();
        return;
        }
    else if (_convertedSensorID == data.at(5).toInt(nullptr, 16))
    {
        _value = (data.at(6)+ data.at(7)).toInt(nullptr, 16)/10;
        emit valueChanged();
        return;
